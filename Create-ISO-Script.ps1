...
function Get-Directory-Name($initialDirectory)
{   
    [System.Reflection.Assembly]::LoadWithPartialName("System.windows.forms") |
    Out-Null
...
Write-Host 'Please specify the source directory...'
Start-Sleep -s 1

$source_dir = Get-Directory-Name($cwd)
If (!$source_dir -Or -Not (Test-Path $source_dir)) {
    Write-Error -message "Source Path not valid."
    exit 0
}

Write-Host 'Please specify the output file...'
Start-Sleep -s 1

$output = Get-FileName($cwd, "Output File Path (.iso)")
If (!$output -Or -Not (Test-Path (Split-Path -path $output -parent))) {
    Write-Error -message "Output File Path not valid."
    exit 0
}

Get-ChildItem "$source_dir" | New-IsoFile -path "$output"